# Game Mechanics Updates - Complete Specification (updated)

## Level Health Gain
- Health should only be restored every 10 levels instead of every 1

## Level Progression
- The points needed to progress from level to level should be doubled what it currently is and slowly increase exponentially
...
- Level difficulty progression should happen 5% faster
...
- The regular gun rate of fire should be faster if manually triggered rather than if the user manually holds the button down
  - Tap-firing gets a bonus (faster than current rate)
...
- Missiles should slowly increase speed the further they fly
...
- Once all the other enemies are gone from the screen, cartoonish nuclear waste barrels should start floating down with mechanics like the burgers
  - These should have a sort of gravity that gently attracts them to the user so that if the user doesn't move at all they will be much more likely to impact the user
...
Game mechanics updates overview (orginal)
...
- The points needed to progress from level to level should be doubled what it currently is and slowly increase exponentially. 
- A HUD/UI indicator should be added to indicate to the user their progress in the current level 
- Level difficultly progression should happen 5% faster
## Weapon changes
Major changes:
- Upgraded weapons will now have a limited ammo stock rather than a time limit
- Only one power up should appear when the trigger event is reached. Which upgrade is spawned should be random, with logic to ensure there is an even amount of spawns of each over every 100 spawns. 
Notes: 
- Cool downs still apply
Weapons:
- Regular Gun
	- The regular gun rate of fire should be faster if manually triggered rather than if the user manually holds the button down
- bazooka
	...
	- Only 5 missiles per power up
- shotgun
	- Bullets should have a slightly different color from the regular gun. closer to a sliver/metallic look
	- 5 rounds per load, with 10 reloads. Each reload takes 3 seconds. 
	- Rate of fire should be half of what it is currently
- laser
	- when a laser bullet ricochets, it should be cloned 3 times and the 3 clones should fire off in different directions approximately 33 degrees from each other (as measured from the center of the target). The exact separation and directions should be randomized.
	- The clones each have the same effect on any target and the user. 
	- laser will have 1,000 "bullets"
## Enemies 
### Crabby patties (burgers)
- The burgers should have 10% more variety in size, speed, and direction 
- Burgers should bounce off each other with a small amount of randomness to keep directions a little unpredictable. 
### Atomic Lobsters (NEW)
- Every 3rd level (starting with the 3rd level) a random number of "atomic lobsters" should fly onto the screen from the left and right sides and then fall to the bottom in a similar speed and fashion as the burgers. 
- The lobsters should have mechanics similar to fighter jet with a dual slow fire rate cannons in its pincers. (1/3 of the rate of the user's standard weapon). The cannon "bullets" should be noticeably larger than the users bullets and look differently as well. These bullets travel horizontally in a straight line.  Note: there shouldn't actual cannon graphics, the "bullets" should just come out of the pincers like the bullets come out of the user. 
- The lobsters should actively target the user. 
- The lobsters should follow the same visual style as the rest of the game but be simplified, cartoon, red, 2d line art lobsters with a top down perspective (like how a lobster would look on the ground if you were standing over it). They should have some sort of "outfit" with the nuclear symbol on it. 
### Nuclear waste barrels
- every 10 levels (starting on the 11th), the user should enter a special level. 
- Other enemies (burgers, lobsters) should not spawn during during this level. Existing ones will go away as they normally would. 
- Once all the other enemies are gone from the screen, cartoonish nuclear waste barrels should start floating down with mechanics like the burgers. These should have a sort of gravity that gently attracts them to the user so that if the user doesn't move at all they will be much more likely to impact the user. 
- The barrels do 2x damage than the patties. 
- When the barrels are killed by the user they have splash damage and can do 1x damage as the patties if they are shot too close to the user. 
- Spawn a random amount of barrels between 10 and 50. 
- Disable the HUD item tracking the level progression so the total number of barrels are a secret to the user. 
- make some sort of drastic animation to indicate to the user something big is about to happen before the barrels spawn
- Make celebration animation once the barrel level has been defeated before moving on to the next level. There should be a pause here and the user should have to click or take action to continue. This will serve as a natural break in the game. 
