...
export class DatabaseManager {
   ...
   */
  private calculateChecksum(data: Omit<SaveGame, 'id' | 'checksum'>): string {
   ...
   */
  private verifyChecksum(save: SaveGame): boolean {
   ...
   * Save game to a slot
   */
  saveGame(slotName: string, data: Omit<SaveGame, 'id' | 'slotName' | 'checksum' | 'timestamp'>): boolean {
    if (!this.db) throw new Error('Database not initialized');
   ...
   */
  loadGame(slotName: string): SaveGame | null {
    if (!this.db) throw new Error('Database not initialized');
    ...
    try {
      ...
      const row = stmt.get(slotName) as any;
      if (!row) return null;
   ...
   * Add a high score
   */
  addHighScore(score: HighScore): boolean {
    if (!this.db) throw new Error('Database not initialized');
   ...
   * Get top high scores
   */
  getHighScores(limit = 10): HighScore[] {
    if (!this.db) throw new Error('Database not initialized');
   ...
   */
  updateGameStats(updates: Partial<GameStats>): boolean {
    if (!this.db) throw new Error('Database not initialized');
    ...
    try {
      ...
      if (data.settings) {
        for (const [key, value] of Object.entries(data.settings)) {
          this.setSetting(key, value as string);
        }
      }
