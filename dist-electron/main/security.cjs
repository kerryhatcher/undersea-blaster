...
const CSP_PRODUCTION = CSP_DIRECTIVES.filter(directive => !directive.includes('localhost') && !directive.includes('ws:')).join('; ');
...
class SecurityManager {
    ...
    configurePermissions(window) {
        ...
        window.webContents.session.setPermissionCheckHandler((webContents, permission, requestingOrigin) => {
            ...
            if (!this.isDevelopment) {
                const appOrigin = `file://${electron_1.app.getAppPath()}`;
    ...
    configureAppSecurity() {
        ...
        if (!this.isDevelopment) {
            ...
            electron_1.app.commandLine.appendSwitch('disable-remote-module');
            // Disable node integration in subframes
            electron_1.app.commandLine.appendSwitch('disable-node-integration-in-subframes');
        }
    }
}
